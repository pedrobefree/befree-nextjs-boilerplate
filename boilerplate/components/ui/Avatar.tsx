...
const AvatarOnlineIndicator = ({ status }: { status: "online" | "offline" }) => <div className={cx("h-2 w-2 rounded-full absolute bottom-0 right-0 ring-2 ring-white", status === "online" ? "bg-green-500" : "bg-gray-300")} />;
const VerifiedTick = ({ className: _className }: { className?: string }) => null;
...
export const Avatar = ({
    ...
    const renderBadgeContent = () => {
        if (status) {
            return <AvatarOnlineIndicator status={status} />;
        }

        if (verified) {
            return (
                <VerifiedTick className="absolute -bottom-0.5 -right-0.5 ring-2 ring-white" />
            );
        }
